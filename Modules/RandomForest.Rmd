...
We run the random forest algorithm without any model tuning.
...
```
The confusion matrix shows that our tuned matrix is very slightly better than the base model but it is mostly the same.

```{r rf_save, include=FALSE}
saveRDS(tuned_rf, "rf_tuned.rds")
